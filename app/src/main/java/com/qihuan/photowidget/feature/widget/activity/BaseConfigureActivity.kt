...
import com.qihuan.photowidget.feature.widget.adapter.WidgetFrameResourceAdapter
import com.qihuan.photowidget.feature.widget.adapter.WidgetPhotoAdapter
...
abstract class BaseConfigureActivity : AppCompatActivity() {
    ...
    private val viewModel by viewModel<ConfigureViewModel> {
        parameterArrayOf(appWidgetId, widgetType)
    }
    ...
    private fun saveWidget() {
        ...
        lifecycleScope.launch {
            ...
            if (result is SaveWidgetResult.Fail) {
    ...
    private fun launchOpenAppActivity() {
        appSelectResult.launch(
            Intent(Intent.ACTION_VIEW, Uri.parse("qihuan://photowidget/link/app")).apply {
                putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, appWidgetId)
            })
    }
    ...
    private fun launchOpenLinkActivity() {
        appSelectResult.launch(
            Intent(Intent.ACTION_VIEW, Uri.parse("qihuan://photowidget/link/url")).apply {
                putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, appWidgetId)
